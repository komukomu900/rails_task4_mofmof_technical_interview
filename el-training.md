...
- 新入社員 : 本カリキュラムの受講者です。
- サポーター : 新入社員の教育指導助言を行います。また、新入社員と相談して仕様を一緒に決めたりする役割も担います。
...
- タスクに優先順位をつけたい
- ステータス（未着手着手完了）を管理したい
- ステータスでタスクを絞り込みたい
- タスク名タスクの説明文でタスクを検索したい
...
以下の言語ミドルウェアを使って構築していただきたいです（いずれも最新の安定バージョン）。
...
- Heroku
 ...
 性能要求セキュリティ要求は特に定めませんが、一般的な品質で作ってください。
...
- データ構造を考えたら、それをモデル図に手書きで書いてみましょう
  ...
  - `README.md` にテーブルスキーマを記載しましょう（モデル名カラム名データ型）
